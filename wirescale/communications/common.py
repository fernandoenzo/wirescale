...
def systemd_autoremove(config: Union['WGConfig', 'RecoverConfig'], pair: 'ConnectionPair'):
    ...
    args = [config.interface, str(config.suffix), str(pair.peer_ip), remote_pubkey, str(wg_ip), str(int(running_in_remote)), str(config.start_time), str(listen_port), str(int(config.nat)),
            config.remote_interface, str(config.remote_local_port), str(int(config.iptables)), config_file.as_uri(), str(config.recover_tries), str(config.recreate_tries)]
